...
dat_scaled<-dat %>% 
  mutate(mean_ime_percent = mean_ime_percent/100, type = factor(type), reef_area_km2 = log10(reef_area_km2.y+1), island_area_km2 = log10(island_area_km2+1)) %>% 
...
ggpairs(
  dat_scaled %>% 
    select(island_area_km2, reef_area_km2, 
             sst_mean, wave_energy_mean_kw_m1, irradiance_einsteins_m2_d1_mean,
             chl_a_mg_m3_mean, mld, mld_months_deep, ted_mean, ted_sum) %>% na.omit,
  lower = list(continuous = GGally::wrap("points", alpha = 0.5, size=.5)),  # Scatterplots in lower panels
  diag = list(continuous = GGally::wrap("barDiag", bins = 20)),    # Histograms on the diagonal
  upper = list(continuous = GGally::wrap("cor", size = 3))         # Correlations in upper panels
) +
  theme_minimal() 
...
effects <- m %>%
  spread_draws(b_atoll_islandIsland, b_reef_area_km2, b_island_area_km2,
               b_chl_a_mg_m3_mean, b_wave_energy_mean_kw_m1, b_ted_mean, b_ted_sum, b_mld, b_mld_months_deep) %>%  
  ...
  mutate(Variable = str_replace_all(Variable, 'b_', ''),
         var_fac = factor(Variable, 
                          levels = rev(c('atoll_islandIsland','reef_area_km2','island_area_km2',
                                         'ted_mean', 'mld', 'mld_months_deep','chl_a_mg_m3_mean', 'wave_energy_mean_kw_m1','ted_sum'))))
