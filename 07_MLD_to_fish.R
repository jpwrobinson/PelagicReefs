...
b_mld<-eff$med[eff$fg=='Planktivore']/sd(depth$mld_mean) # -0.13 per m MLD

# Extract posterior samples - flux at each island, given geomorphic but excluding site habitat
nd<-plank_scaled %>%  
  group_by(region.col, region, island, geomorphic_type, mld_mean, avg_monthly_mm, 
           reef_area_km2, island_area_km2, population_status, ted_mean) %>% 
  ...
  group_by(island, region, region.col) %>% 
  summarise(base_flux = median(.epred)) %>% 
